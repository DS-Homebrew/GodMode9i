...
TWL_CODE static inline void xor_128(uint32_t *x, const uint32_t *a, const uint32_t *b){
	x[0] = a[0] ^ b[0];
	x[1] = a[1] ^ b[1];
	x[2] = a[2] ^ b[2];
	x[3] = a[3] ^ b[3];
...
TWL_CODE static inline void add_128(uint32_t *a, const uint32_t *b){
...
TWL_CODE static inline void add_128_32(uint32_t *a, uint32_t b){
	a[0] += b;
	if(a[0] < b){
		a[1] += 1;
		if (a[1] == 0) {
...
TWL_CODE static inline void rol42_128(uint32_t *a){
...
TWL_CODE static void dsi_aes_set_key(uint32_t *rk, const uint32_t *console_id, key_mode_t mode) {
	uint32_t key[4];
	switch (mode) {
	case NAND:
		key[0] = console_id[0];
...
TWL_CODE int dsi_sha1_verify(const void *digest_verify, const void *data, unsigned len) {
...
TWL_CODE void dsi_crypt_init(const uint8_t *console_id_be, const uint8_t *emmc_cid, int is3DS) {
...
TWL_CODE static inline void aes_ctr(const uint32_t *rk, const uint32_t *ctr, uint32_t *in, uint32_t *out) {
	uint32_t xor[4];
...
TWL_CODE void dsi_nand_crypt_1(uint8_t* out, const uint8_t* in, uint32_t offset) {
...
TWL_CODE void dsi_nand_crypt(uint8_t* out, const uint8_t* in, uint32_t offset, unsigned count) {
	uint32_t ctr[4] = { nand_ctr_iv[0], nand_ctr_iv[1], nand_ctr_iv[2], nand_ctr_iv[3] };
	add_128_32(ctr, offset);
...
TWL_CODE void dsi_boot2_crypt_set_ctr(uint32_t size_r) {
	boot2_ctr[0] = size_r;
	boot2_ctr[1] = -size_r;
	boot2_ctr[2] = ~size_r;
	boot2_ctr[3] = 0;
...
TWL_CODE void dsi_boot2_crypt(uint8_t* out, const uint8_t* in, unsigned count) {
	for (unsigned i = 0; i < count; ++i) {
...
// why is it called ES?
TWL_CODE int dsi_es_block_crypt(uint8_t *buf, unsigned buf_len, crypt_mode_t mode) {
